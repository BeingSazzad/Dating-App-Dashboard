...
import { ReportsTable, ReportDetailsDialog } from "@/components/reports";
import {
  useGetReportsQuery,
...
export function ReportsPage() {
  ...
  const handleConfirmBan = async () => {
    if (!banTarget) return;
    await banUser(banTarget.reportedUserId)
      .unwrap()
      .catch(() => undefined);
    setBanTarget(null);
  };
    ...
    <div className="space-y-6">
      ...
      <Dialog open={imageModalOpen} onOpenChange={setImageModalOpen}>
        <DialogContent className="max-w-md">
          ...
          {selectedImage && (
            <img src={selectedImage} alt="Report" className="w-full h-auto rounded" />
          )}
          <DialogFooter>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to permanently ban ${banTarget?.reportedUserName}? This will restrict their account access and resolve all their pending reports.`}
      ...
      {/* Report Details & Profile Modal */}
      <ReportDetailsDialog
        report={selectedReport}
        open={Boolean(selectedReport)}
        onOpenChange={(o) => !o && setSelectedReport(null)}
        onResolveClick={(r) => {
          updateReport({ id: r.id, status: "resolved" });
          setSelectedReport(null);
        }}
        onIgnoreClick={(r) => {
          updateReport({ id: r.id, status: "ignored" });
          setSelectedReport(null);
        }}
        onWarnClick={(r) => {
          setWarnTarget(r);
